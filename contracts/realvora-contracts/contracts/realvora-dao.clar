...
(define-public (create-proposal
  ...
  (let (
    ...
    (voting-end (+ stacks-block-height VOTING_PERIOD))
  )
...
(define-public (vote (proposal-id uint) (vote-for bool))
  (let (
    ...
    (asserts! (< stacks-block-height (get voting-end proposal-data)) ERR_VOTING_ENDED)
    (asserts! (>= stacks-block-height (get voting-start proposal-data)) ERR_VOTING_ACTIVE)
...
(define-public (execute-proposal (proposal-id uint))
  (let (
    ...
    (let ((proposal-type (get proposal-type proposal-data)))
      (if (is-eq proposal-type PROPOSAL_TYPE_TREASURY_ALLOCATION)
        (execute-treasury-allocation proposal-data)
        (if (is-eq proposal-type PROPOSAL_TYPE_PROTOCOL_UPGRADE)
          (execute-protocol-upgrade proposal-data)
...
(define-public (update-voting-power (user principal) (new-power uint))
  (begin
    ...
    (map-set voting-power
      { user: user }
      { power: new-power, last-updated: stacks-block-height }
    )
